...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function allocateOTOfficers(count, otStart, otEnd) {
        ...
        // If no main officers exist, skip the floor check — every OT officer can break.
        const maxGaps = {};
        nonBikeZones.forEach(z => {
            ...
            for (let t = gapWinStart; t < gapWinEnd; t++) {
                ...
                if (active > 0) anyMain = true;
                if (active < minMain) minMain = active;
            }

            // If no main officers at all in zone, all OT officers may have breaks
            maxGaps[z.name] = anyMain
                ? Math.max(0, minMain + zoneQuota[z.name] - minReq)
                : zoneQuota[z.name];
        });
